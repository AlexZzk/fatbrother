...
> 云函数名：`order`
> Sprint：S5（C端下单）、S6（B端接单）、S7（支付/退款/分账）
...
|--------|------|------|--------|------|
| `create` | 创建订单（支持模拟/真实支付） | C端用户 | S5/S7 | **已完成** |
...
| `complete` | 商户确认完成（自动触发分账） | B端商户 | S6/S7 | **已完成** |
| `autoCancel` | 超时自动取消 | 定时触发 | S6 | **已完成** |
| `createPayment` | 重新发起支付 | C端用户 | S7 | **已完成** |
| `paymentNotify` | 支付回调处理 | 微信支付 | S7 | **已完成** |
| `createRefund` | 发起退款 | 系统 | S7 | **已完成** |
| `settlement` | 订单分账结算 | 系统 | S7 | **已完成** |
